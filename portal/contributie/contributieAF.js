...
$("#formular-container>form").submit(function(e) {
    ...
    const fisier_propunere = propunereID + "-af-" + getUserId()+"-propunere."+FISIER_PROPUNERE_DOC.extension
    const fisier_notabibliografica =propunereID + "-af-" + getUserId() +"-notabibliografica."+FISIER_NOTABIBLIOGRAFICA_DOC.extension
    ...
    //upload data
    uploadDataToAF(realtimeDatabaseForm)

    //upload file

    uploadFileToAF(FISIER_PROPUNERE_DOC.file,fisier_propunere)
    uploadFileToAF(FISIER_NOTABIBLIOGRAFICA_DOC.file,fisier_notabibliografica)
    ...
    database.ref(`users/${getUserId()}`).once("value")
        .then((snapshot)=>{
            ...
            if(user.contributions)
                user.contributions.push(propunereID+'-af')
            else
                user.contributions = [propunereID+'-af']

            if(!user.ranks["aaf"]) user.ranks["aaf"] = true
...
function uploadFileToAF(file,name){
    var storageRef = firebase.storage().ref('AF/'+ name);
    storageRef.put(file)
        .then((snapshot)=>{
            //pass
            console.log("done")
...
function uploadDataToAF(object) {
    var ref = database.ref("AF/propuneri/"+object.id)
    ref.set(object)
        .then((snapshot)=>{
